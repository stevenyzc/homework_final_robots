...
void Manager::add_new_robot(const std::shared_ptr<Robot>& robot)
{
...
auto Manager::find_working_robot(int team,int id) // 查找正常机器人
{
    ...
    {
        if ((*it)->get_team()==team && (*it)->get_id()==id) // 找到机器人
            break;
    }
    return it; // 返回迭代器指针
}
    ...
    for (it=working.begin();it!=working.end();) // 遍历所有正常机器人
    {
        if ((*it)->get_heat()>0) // 机器人热量为正
        {
            if ( (*it)->get_heat() > (*it)->get_max_heat() ) // 机器人热量大于热量上限
            {
                ...
                if ((*it)->get_blood()<=0) // 被击毁
                {
                    add_destroyed_robot((*it)->get_max_blood(),(*it)->get_max_heat(),(*it)->get_blood(),
                                        (*it)->get_heat(),(*it)->get_team(),(*it)->get_id()); // 将该机器人移入被击毁的机器人列表
                    if (flag==1) // 血量下降的方式为：在时间结束前或时间结束时恰好机器人热量减小到最大热量
                        reduce_blood = (*it)->get_blood() + (*it)->get_max_heat() - (*it)->get_heat();
                    else // 血量下降的方式为：在时间结束时机器人热量未减小到最大热量
                        reduce_blood = (*it)->get_blood() + time;
                    // 按照reduce_blood的值从小到大有序插入同一段机器人枪口热量降低的时间内被摧毁的机器人
                    reduce_heat_destroyed.insert ( std::make_pair
                                                           ( reduce_blood , std::make_pair ( (*it)->get_team() , (*it)->get_id() ) ) );
                ...
                {
                    (*it)->set_heat ( (*it)->get_heat() - time ) ; // 每秒钟降低1点热量，共time点
                    if ((*it)->get_heat()<0) // 热量不能为负数
                        (*it)->set_heat(0); // 设定热量为0
                    it++; // 迭代器后移一位
                }
            }
            else // 机器人热量小于等于热量上限
...
void Manager::command_A(int team,int id,int type) // 添加一个新的机器人（type=0为步兵，type=1为工程）
{
    ...
    {
        add_working_robot((*it)->get_max_blood(),(*it)->get_max_heat(),
                          (*it)->get_team(),(*it)->get_id()); // 将该机器人移入正常机器人列表
...
void Manager::command_F(int team,int id,int damage) // 对该机器人造成damage点伤害
{
    ...
    {
        (*it)->set_blood ( (*it)->get_blood() - damage ) ; // 对该机器人造成damage点伤害（血量下降damage点）
        if ((*it)->get_blood()<=0) // 被击毁
        {
            add_destroyed_robot((*it)->get_max_blood(),(*it)->get_max_heat(),(*it)->get_blood(),
                                (*it)->get_heat(),(*it)->get_team(),(*it)->get_id()); // 将该机器人移入被击毁的机器人列表
            std::cout<<"D "<<(*it)->get_team()<<' '<<(*it)->get_id()<<std::endl; // 输出：D [所属队伍] [机器人标识符]
...
void Manager::command_H(int team,int id,int add_heat) // 该机器人枪口热量上升指定值add_heat
{
    ...
    if (it!=working.end()) // 找到机器人
    {
        bool flag=whether_Engineer((*it)->get_max_heat()); // 判断是否为工程
        if (flag==0) // 是步兵
            (*it)->set_heat ( (*it)->get_heat() + add_heat ); // 该机器人枪口热量上升指定值add_heat
    }
}
...
void Manager::command_U(int team,int id,int lv) // 将该机器人设定为该等级lv
{
    ...
    {
        bool flag=whether_Engineer((*it)->get_max_heat()); // 判断是否为工程
        if (flag==0 && lv>(*it)->get_max_heat()/100) // 是步兵且目标等级大于当前等级（等级=热量上限/100）
        {
            (*it)->set_max_blood(100*lv-50); // 血量上限=100*等级-50（只考虑等级2、3）
            (*it)->set_max_heat(100*lv); // 热量上限=100*等级
            (*it)->set_blood ( (*it)->get_max_blood() ) ; // 将血量重置为血量上限
        }
    }
}